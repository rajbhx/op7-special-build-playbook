...
before every task. That is for humans. For agents, use the notes layer:

```bash
python3 scripts/lookup.py install testonly   # ranked matches, one line each
python3 scripts/lookup.py --id C1            # full detail for exactly one entry
python3 scripts/lookup.py --sessions         # conversation digests
python3 scripts/lookup.py --tags             # all curated search tags
grep -i shizuku notes/*/INDEX.md             # ultra-cheap fallback, no python
```

Layout: `notes/<slug>/INDEX.md` = keyword → entry ids (grep this),
`notes/<slug>/entries/<id>.md` = one file per problem+solution,
`notes/<slug>/SESSIONS.md` = synced conversation digests,
`notes/<slug>/CONVERSATIONS.md` = typed knowledge extracted from Codex chats
(RULE/DECISION/REQUEST/GOTCHA/GOAL — never raw transcripts),
`notes/<slug>/index.json` = machine-readable index used by `lookup.py`.
All generated by `scripts/build_notes.py` from the project logs; never hand-edit.
...
- Entry schema per section: `id` (unique), `problem`, `cause`, `solution`,
  plus optional `tags: [word1, word2]` to boost searchability (lowercase, no spaces).
  If tags are omitted, keywords are derived from the text automatically.
...
- Generated, do NOT hand-edit: `projects/*/README.md`, `projects/README.md`,
  `notes/` (whole layer), `docs/09-field-notes-journey.md`.
